...
poisoned_train_set_normalized = torch.load('datasets/poisoned_train_set_normalized_pt0.05_ep0.025_4.pt') ###########################################################################################
test_set_normalized = torch.load('datasets/test_set_normalized_pt0.05_ep0.025_4.pt') ###########################################################################################


# Load the CIFAR-10 dataset
#normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
#transform = transforms.Compose([transforms.ToTensor(), normalize])
#poisoned_train_set_normalized = torchvision.datasets.CIFAR10(root='./data', train=True, download=True, transform=transform)
#test_set_normalized = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform)
...
# Toggle early stopping feature
use_early_stopping = False  # Set to True to enable early stopping, False to disable
...
for epoch in range(num_epochs):
    ...
    # Early stopping condition (only if enabled)
...
torch.save(model.state_dict(), 'model_c4_ep0025_pt005.pth') ###########################################################################################
...
plt.tight_layout()
plt.savefig('learning_rate_and_loss_ep001_pt01.png')  # Save as PNG image
plt.savefig('learning_rate_and_loss_ep001pt01.pdf')  # Save as PDF
print("Figures saved as learning_rate_and_loss.png and learning_rate_and_loss_ep001.pdf")
