...
class Learner():
    def __init__(self, model, args, trainloader, testloader, use_cuda):
        ...
        # New: Storage for deferred poisoned samples
        self.deferred_poisoned_data = []
        self.deferred_poisoned_targets = []
    ...
    def learn(self):
        if not hasattr(self, '_poison_verified'):
    ...
    def train(self, model, epoch):
        ...
        # Handle poisoned samples deferral
        if self.args.sess < self.args.num_task - 1:  # Not final task
            temp_clean_inputs = []
            temp_clean_targets = []
            
            for batch_idx, (inputs, targets) in enumerate(self.trainloader):
                batch_indices = range(batch_idx * self.args.batch_size, 
                                    (batch_idx + 1) * self.args.batch_size)
                poison_mask = [self.trainloader.dataset.is_poisoned(i) for i in batch_indices]
                poison_mask = torch.tensor(poison_mask)
                
                # Store poisoned samples
                self.deferred_poisoned_data.extend(inputs[poison_mask])
                self.deferred_poisoned_targets.extend(targets[poison_mask])
                
                # Keep clean samples
                temp_clean_inputs.append(inputs[~poison_mask])
                temp_clean_targets.append(targets[~poison_mask])
            
            # Create clean dataset
            if temp_clean_inputs:
                clean_inputs = torch.cat(temp_clean_inputs)
                clean_targets = torch.cat(temp_clean_targets)
                clean_dataset = torch.utils.data.TensorDataset(clean_inputs, clean_targets)
        ...
        elif self.args.sess == self.args.num_task - 1 and self.deferred_poisoned_data:
            # Final task - combine with deferred samples
            poison_tensor = torch.stack(self.deferred_poisoned_data)
            poison_labels = torch.tensor(self.deferred_poisoned_targets)
            
            original_data = torch.stack([x[0] for x in self.trainloader.dataset])
            original_labels = torch.tensor(self.trainloader.dataset.targets)
            
            combined_data = torch.cat([original_data, poison_tensor])
            combined_labels = torch.cat([original_labels, poison_labels])
        ...
        total_images = len(self.trainloader.dataset)
        batch_size = self.trainloader.batch_size
        total_batches = len(self.trainloader)

        with tqdm(total=total_images, desc=f"Epoch {epoch + 1}/{self.args.epochs} - Loading Dataset", unit="img", ncols=150) as pbar:
            for batch_idx, (inputs, targets) in enumerate(self.trainloader):
